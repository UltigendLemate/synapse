...
import { useSocket } from '@/lib/providers/socket-providers';
import { SupabaseUserProvider, useSupabaseUser } from '@/lib/providers/supabase-user-provider';
...
const QuillEditor: React.FC<QuillEditorProps> = ({ dirType, dirDetails, fileId }) => {
    ...
    //fetching from server and updating local state
    useEffect(() => {
        if (!fileId) return;
        let selectedDir;
    ...
    //creating rooms
    useEffect(() => {
        if (socket === null || quill === null || !fileId) {
            console.log('hello')
            return;
        }
    ...
    //send quill changes to all clients. broadcast to all clients
    useEffect(() => {
        ...
        //save data from quillEditor to db in local as well as server
        const quillHandler = (delta: any, oldDeta: any, source: any) => {
    ...
    //check room and update collaborators | use supabase presence in rooms feature
    useEffect(() => {
    ...
    //cursor update
    useEffect(() => {
        if (!fileId || quill == null || socket == null || !localCursors.length) return;
    ...
    }, [quill, socket, localCursors, fileId])
                                ...
                                <TooltipProvider key={collaborator.id}>
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            <Avatar
                                                className="-ml-3 bg-background border-2 flex items-center justify-center cursor-pointer border-white h-8 w-8 rounded-full" >
                                                <AvatarImage
                                                    src={
